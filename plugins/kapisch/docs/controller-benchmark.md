...
For `durable-fix`, retain the ordered JSONL evidence within one `run_id`: initial implementer invocation, reviewer `do-not-approve` with positive findings, a later implementer fix, reviewer `approve` re-review, then reviewer `ready`. Invocation numbers establish this sequence; each phase remains a separate row. Behavioral and resume evidence is likewise complete within each run.
