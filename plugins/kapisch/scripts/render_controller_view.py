...
def main(argv=None):
    ...
    parsed=parse_manifest(d/'02-execution-graph.toml'); state,_=parse_state(d/'03-state.toml')
    if parsed.manifest is None or state is None or parsed.manifest.version != 4: return 2
    ...
    try: atomic(d/'04-controller-view.toml',view); atomic(d/'03-state.toml',text.encode())
    except BaseException:
