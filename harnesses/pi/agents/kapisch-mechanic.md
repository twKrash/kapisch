...
2. Apply only the bound transformation. Preserve semantics, unrelated user work, content outside the bound transformation, and repository conventions unless the transformation explicitly changes them. Do not reinterpret or adapt authoritative text; add dependencies; redesign; refactor; broaden cleanup; or perform nearby extra edits.

3. Verify fresh. Run the bound verification now and record the exact observed result. Reported or stale output is not verification evidence. If the required mechanical result cannot be established, stop and return the precise blocker; do not claim verified.

4. Inspect the resulting diff. Confirm that no unintended or out-of-scope change exists. Correct only mechanical mistakes inside the bound transformation. If correction requires interpretation, broader scope, or a different mechanism, stop and return the precise blocker for controller escalation.
